"""
Streamlit UI for interacting with the trained language model.
"""
...
def load_model(model_path: str, model_type: str = "decoder_only"):
    ...
    # Load model weights
    checkpoint = torch.load(f"{model_path}.pt", map_location=device)
    ...
    if load_button:
        with st.spinner("Loading model and tokenizer..."):
            try:
                ...
                # Load model
                st.session_state.model = load_model(model_path, model_type)
                
                # Get model info
                st.session_state.model_info = {
                    "parameter_count": st.session_state.model.get_parameter_count(),
                    "config": st.session_state.model.config,
                    "device": next(st.session_state.model.parameters()).device.type
                }
            ...
            if not st.session_state.model_loaded:
                ...
                try:
                    ...
                    if not prompts:
                        ...
                        # Get device once for all prompts
                        device = next(st.session_state.model.parameters()).device
